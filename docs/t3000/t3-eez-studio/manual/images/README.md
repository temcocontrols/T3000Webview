...
All figures are numbered `NN-area-slug.png` in the order they appear in the manual. Drop
each file here using **exactly** the filename below — the chapters already reference these
paths, so no documentation edit is needed once an image exists.

## Done

| Filename | Shows |
|---|---|
| `01-hub-overview.png` | The **Design Hub** dashboard (hero, device bar, tiles, Project History). |
...
| `15-editor-styles-fonts.png` | 3.4 | The **Styles / Fonts / Bitmaps** tabs. |
| `16-flow-editor.png` | 4.1 | A small **Flow** graph (2–3 components wired together). |
| `17-flow-palette.png` | 4.2 | The Flow **components palette** with its groups. |
| `18-flow-widget-event.png` | 4.3 | A widget's **event handler** bound to a flow. |
| `19-preview-run.png` | 5.1 | The editor in **Run** mode with the UI showing in the canvas. |
| `20-simulator.png` | 5.2 | The **Full Simulator** window (F7), if enabled. |
| `21-bind-device.png` | 5.3 | The **Bind device** dialog with a device and its building/floor/room. |
| `22-deploy-drawer.png` | 5.4 | The **Deploy to Device** drawer mid-deploy — step list and progress. |
| `23-deploy-log.png` | 5.5 | A finished deploy with its **step log** (save → export → compare → push → record). |
| `24-project-detail.png` | 5.6 | The **Project Detail** page — preview, statistics, snapshot/compare. |

## Tips

- Capture at a normal window width (≈1440 px) so text stays legible.
- Crop tightly to the area named in the "What to capture" column.
