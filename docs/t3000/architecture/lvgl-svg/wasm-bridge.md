...
svgScnEmitObj       one SceneObject: identity, area, textArea, clipChildren, transform, scroll, parts[]
...
| `bgSymbol` | the codepoint string of a SYMBOL background image (`bg_image_src` whose first byte is ≥ 0x80) | the default theme draws a **checked checkbox's tick** as `bg_image_src = LV_SYMBOL_OK` — a font glyph, not a bitmap — in the part's own text font and colour (`lv_obj_draw.c`). Without it a checked box came out empty; `scene-dump.ts` maps it to `SceneBgImage.symbol` and the renderer traces the ones it knows as vector paths (an untraced codepoint still reaches the SVG as text) |
