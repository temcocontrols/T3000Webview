 ...
 *
 * Display and edit BACnet output points
 */
...
import type { OutputPoint } from '@common/react/types/bacnet';
...
export const OutputsPage: React.FC = () => {
  ...
  const { fetchOutputs } = useBacnetApi();
  const outputs = useBacnetStore((state) => state.outputs);
  const [loading, setLoading] = useState(false);
  const [selectedPoint, setSelectedPoint] = useState<OutputPoint | null>(null);
  ...
  const loadOutputs = async () => {
    ...
    setLoading(true);
    try {
      await fetchOutputs(selectedDevice.id);
    } finally {
      setLoading(false);
    }
  };

  const handleSavePoint = async (point: OutputPoint) => {
    // TODO: Implement save via API
    console.log('Saving point:', point);
    setSelectedPoint(null);
  };

  const columns: Column<OutputPoint>[] = [
    {
      key: 'index',
      label: '#',
      sortable: true,
    ...
    return (
      <div className={styles.container}>
        <EmptyState
          ...
          message="Please select a device from the tree to view outputs"
        />
      </div>
    );
  }
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <div className={styles.content}>
          {outputs.length === 0 ? (
            <EmptyState
              title="No Outputs"
              message="No output points found for this device"
            />
          ) : (
            <DataTable
              columns={columns}
