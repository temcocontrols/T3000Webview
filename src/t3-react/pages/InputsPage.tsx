/**
 * InputsPage Component
 *
 * Display and edit BACnet input points
 */
...
import type { InputPoint } from '@common/react/types/bacnet';
...
export const InputsPage: React.FC = () => {
  ...
  const { fetchInputs } = useBacnetApi();
  const inputs = useBacnetStore((state) => state.inputs);
  ...
  const loadInputs = async () => {
    ...
    setLoading(true);
    try {
      await fetchInputs(selectedDevice.id);
    } finally {
      setLoading(false);
    }
  };

  const handleSavePoint = async (point: InputPoint) => {
    // TODO: Implement save via API
    console.log('Saving point:', point);
    setSelectedPoint(null);
  };

  const columns: Column<InputPoint>[] = [
    {
      key: 'index',
      label: '#',
      sortable: true,
    ...
    return (
      <div className={styles.container}>
        <EmptyState
          ...
          message="Please select a device from the tree to view inputs"
        />
      </div>
    );
  }
  ...
  if (loading) {
    return <LoadingSpinner message="Loading inputs..." />;
  }
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <div className={styles.content}>
          {inputs.length === 0 ? (
            <EmptyState
              title="No Inputs"
              message="No input points found for this device"
            />
          ) : (
            <DataTable
              columns={columns}
