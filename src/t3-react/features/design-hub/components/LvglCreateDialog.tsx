...
export const LvglCreateDialog: React.FC<{
  ...
  const runImport = async () => {
    ...
    try {
      push(`📋 Importing from ${device.name}`);
      push(`   IP: ${device.ip || '(not on record)'}  SN: ${device.serialNumber}`);

      // Step 0 — create project folder skeleton
      const projectDir = `project/${device.name}`;
      const stagingDir = `${projectDir}/device-import`;
      push('⏳ Step 0 — Creating project folder...');
      await fetch(`/api/eez-studio/make-folder`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: stagingDir }),
      });
      push('✅ Step 0 — Project folder ready');

      // Step 1 — connect via direct REST
      push('⏳ Step 1 — Connecting to device...');
      ...
      push(`✅ Step 1 — Connected via ${conn.mode.toUpperCase()}${device.ip ? ` (${device.ip})` : ''}`);

      // Step 2 — device info
      push('⏳ Step 2 — Fetching device info...');
      const info = await client.getDeviceInfo();
      push(
        `✅ Step 2 — ${info.screen_count} screens, ${info.image_count} images, ${info.screen_size.width}x${info.screen_size.height}`
      );

      // Step 3 — load each screen
      push(`⏳ Step 3 — Loading ${info.screen_count} screens...`);
      ...
      for (const screenName of info.screens) {
        ...
        push(`   ${screen.name} — ${kb}KB ✓`);
      }
      push(`✅ Step 3 — Loaded ${stagingScreens.length} screens`);

      // Step 4 — build the .eez-project
      push('⏳ Step 4 — Building project...');
      ...
      if (project.bitmaps?.length) {
        ...
        push(`⏳ Step 4 — Extracting ${project.bitmaps.length} images...`);
        let loaded = 0;
        for (const bmp of project.bitmaps) {
          ...
          try {
            ...
            /* skip images not present in firmware */
          }
        }
        push(`   ${loaded}/${project.bitmaps.length} images saved to ${imgDir} ✓`);
      }

      // Step 5 — save the .eez-project
      const projectPath = `${projectDir}/${device.name}.eez-project`;
      push('⏳ Step 5 — Saving project...');
      const saveResp = await fetch(
        `/api/eez-studio/write-text-file?path=${encodeURIComponent(projectPath)}`,
        { method: 'POST', body: JSON.stringify(project, null, 2) }
      );
      if (!saveResp.ok) throw new Error('Failed to save project');
      push('✅ Step 5 — Project saved');

      // Bind project → source device (used by "Deploy to Device")
      try {
        const { setDeviceBinding } = await import('project-editor/build/device-binding');
        ...
        console.error('[LvglCreateDialog] device binding failed:', err);
      }
  ...
  // Parse the log into coarse steps (✅ / ⏳ / ❌ lines) for the status list
  const steps = useMemo(() => {
                  ...
                  <div className={styles.importPanel}>
                    <div className={styles.importSteps}>
                      ...
                      {steps.map((s, i) => (
                        ...
                        >
                          {s.status === 'done' && (
                            <CheckmarkCircleRegular style={{ color: '#1e8e3e', flexShrink: 0 }} />
                          )}
                          {s.status === 'active' && <Spinner size="tiny" />}
                          {s.status === 'error' && (
                            <DismissCircleRegular style={{ color: '#d13438', flexShrink: 0 }} />
                          )}
