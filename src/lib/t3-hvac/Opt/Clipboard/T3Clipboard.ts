...
class T3Clipboard {
  ...
  static Init(options?) {
    ...
    // Safety check: Skip initialization if required DOM elements don't exist (e.g., in React)
    if (!this.clipboardInputElement || this.clipboardInputElement.length === 0) {
      LogUtil.Debug('T3Clipboard: Required DOM elements not found, skipping clipboard initialization');
      return;
    }
