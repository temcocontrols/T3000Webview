...
class PolyLine extends BaseLine {
  ...
  CalcTextPosition(svgShape) {
    console.log("S.PolyLine: CalcTextPosition input", { svgShape });
    ...
    // Copy polyline and update its frame (Visio group update).
    let polyLineCopy = Utils1.DeepCopy(this);
    let visioResult = new SDF.Result();
    polyLineCopy.BlockID = -1;
    SDF.SD_UpdateVisioGroupFrame(polyLineCopy, visioResult, true);

    // Get the polyline points without transformation.
    const polyPoints = polyLineCopy.GetPolyPoints(ConstantData.Defines.NPOLYPTS, false, false, false, polyPointsIndices);
    const shapeCenter = {
    ...
    for (let segCount = 0; segCount < totalSegmentCount; segCount++) {
      ...
      if (Utils2.pointInRect(segmentRect, candidateRectPoint1) || Utils2.pointInRect(segmentRect, candidateRectPoint2)) {
        ...
        for (let subIndex = startIndex + 1; subIndex < nextIndex; subIndex++) {
          ...
          let angleRadians = GlobalData.optManager.SD_GetClockwiseAngleBetween2PointsInRadians(firstPoint, secondPoint);
          let angleDegrees = (angleRadians * (180 / ConstantData.Geometry.PI)) % 180;
    ...
    console.log("S.PolyLine: CalcTextPosition output", {
  ...
  WriteSDFAttributes(writer, options, usePolyListDimensions) {
    console.log("S.PolyLine: WriteSDFAttributes input", { writer, options, usePolyListDimensions });
    ...
    // and we are not writing for Visio, then process a possible polygon conversion.
    if (
      ...
      // Get poly points.
      polyPoints = polyConverted.GetPolyPoints(ConstantData.Defines.NPOLYPTS, false, true, false, indexArray);
      ...
      polyPoints = GlobalData.optManager.InflateLine(tempArray, polyObj.StyleRecord.Line.BThick, true, false);
    ...
    if (usePolyListDimensions) {
      polyWidth = SDF.ToSDWinCoords(polyObj.polylist.dim.x, options.coordScaleFactor);
      polyHeight = SDF.ToSDWinCoords(polyObj.polylist.dim.y, options.coordScaleFactor);
      tempOffsetX = polyObj.polylist.offset.x;
      tempOffsetY = polyObj.polylist.offset.y;
    } else {
      polyWidth = SDF.ToSDWinCoords(polyObj.Frame.width, options.coordScaleFactor);
      polyHeight = SDF.ToSDWinCoords(polyObj.Frame.height, options.coordScaleFactor);
      tempOffsetX = polyObj.StartPoint.x - polyObj.Frame.x;
      tempOffsetY = polyObj.StartPoint.y - polyObj.Frame.y;
    }

    // Process for Visio export.
    if (options.WriteVisio) {
      ...
      visioPoly.SimplifyForVisioExport(polyObj);
    }
    ...
    for (segIndex = 0; segIndex < polyObj.polylist.segs.length; segIndex++) {
      var segWinX = SDF.ToSDWinCoords(polyObj.polylist.segs[segIndex].pt.x + tempOffsetX, options.coordScaleFactor);
      var segWinY = SDF.ToSDWinCoords(polyObj.polylist.segs[segIndex].pt.y + tempOffsetY, options.coordScaleFactor);
      tempLineType = SDF.LineTypetoWin32type(
      ...
      if (polyObj.polylist.segs[segIndex].LineType === ConstantData.LineType.ARCLINE) {
        if (segIndex > 0 &&
      ...
      if (options.WriteVisio || options.WriteWin32) {
        var segStruct = {
          ...
          dimDeflection: SDF.ToSDWinCoords(polyObj.polylist.segs[segIndex].dimDeflection, options.coordScaleFactor),
          flags: polyObj.polylist.segs[segIndex].flags,
          weight: polyObj.polylist.segs[segIndex].weight
        };
        if (options.WriteVisio && polyObj.polylist.segs[segIndex].LineType === ConstantData.LineType.ARCSEGLINE) {
          // Retrieve ellipse points for Visio export.
          var ellipsePoints = this.Pr_GetEllipsePoints(5, true, segIndex, polyObj.polylist.segs[segIndex].param, polyObj.polylist.segs[segIndex].ShortRef, 1, 1);
          if (ellipsePoints && ellipsePoints.length === 5) {
            segStruct.pt.x = SDF.ToSDWinCoords(ellipsePoints[2].x, options.coordScaleFactor);
    ...
    if (!(usePolyListDimensions || (options.WriteVisio && this.LineType !== ConstantData.LineType.POLYLINE))) {
      if (this.DataID >= 0) {
        ...
        if (this.LineTextX) {
          textFlag = ConstantData.TextFlags.SED_TF_AttachC;
        }
        this.TextFlags = Utils2.SetFlag(this.TextFlags, ConstantData.TextFlags.SED_TF_AttachA | ConstantData.TextFlags.SED_TF_AttachB | ConstantData.TextFlags.SED_TF_AttachC | ConstantData.TextFlags.SED_TF_AttachD, false);
      ...
      if ((options.WriteBlocks || options.WriteVisio) && (polyID = this.DataID),
      ...
      SDF.WriteArrowheads(writer, options, this);
    }

    console.log("S.PolyLine: WriteSDFAttributes output");
  }
