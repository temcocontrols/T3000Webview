<template>
  <div class="gauge">
    <svg v-if="height" :viewBox="`0 0 ${RADIUS * 2} ${height}`" height="100%" width="100%"
      ...
      <defs>
        ...
        <linearGradient v-if="hasGradient" :id="`gaugeGradient-${uid}`">
          <stop v-for="(color, index) in gaugeColor" :key="`${color.color}-${index}`" :offset="`${color.offset}%`"
        ...
        <mask :id="`innerCircle-${uid}`">
          ...
          <template v-if="separatorPaths">
            ...
            <path v-for="(separator, index) in separatorPaths" :key="index" :d="separator" fill="black" />
          </template>
        </mask>
      </defs>
      ...
      <g :mask="`url(#innerCircle-${uid})`">
        ...
        <circle v-if="isCircle" :r="RADIUS" :cx="X_CENTER" :cy="Y_CENTER"
          :fill="hasGradient ? `url(#gaugeGradient-${uid})` : gaugeColor" />
        <path v-else :d="basePath" :fill="hasGradient ? `url(#gaugeGradient-${uid})` : gaugeColor" />

        <!-- Draw a circle if the empty gauge has a 360° angle, otherwise draw a path -->
        <circle v-if="modelValue === min && isCircle" :r="RADIUS" :cx="X_CENTER" :cy="Y_CENTER" :fill="baseColor" />
      ...
      <template v-if="scaleLines">
        ...
        <line v-for="(line, index) in scaleLines" :key="`${line.xE}-${index}`" :x1="line.xS" :y1="line.yS" :x2="line.xE"
          :y2="line.yE" stroke-width="1" :stroke="baseColor" />
      </template>
