  ...
  <div class="item-config flex flex-nowrap column" v-if="item">
    <div class="item-config-inner">
      <q-expansion-item class="mb-2 border border-solid border-gray-700" dark default-opened label="General">
        <div class="p-1">
          <div class="grid gap-4 grid-cols-2 mb-4">
            <q-input input-style="width: 100%" @update:model-value="refreshSelecto" label="X"
              v-model.number="item.translate[0]" dark filled type="number" />
            <q-input input-style="width: 100%" @update:model-value="refreshSelecto" label="Y"
          ...
          <div class="flex flex-nowrap items-center mb-2" v-if="item.settings.textColor !== undefined">
            <input type="color" id="text-color-input" v-model="item.settings.textColor" />
            <label class="ml-2" for="text-color-input">Text Color</label>
          </div>
          <div class="w-full mb-2" v-if="item.settings.icon !== undefined">
            <q-select filled dark v-model="item.settings.icon" :options="icons" label="Icon" emit-value map-options>
          ...
          <div class="flex flex-nowrap items-center mb-2" v-if="item.settings.offColor !== undefined">
            <input type="color" id="off-color-input" v-model="item.settings.offColor" />
            <label class="ml-2" for="off-color-input">Off Color</label>
          </div>
          <div class="flex flex-nowrap items-center mb-2" v-if="item.settings.onColor !== undefined">
            <input type="color" id="on-color-input" v-model="item.settings.onColor" />
          ...
          <q-checkbox v-if="!item.t3Entry && item.settings.active !== undefined" dark filled
            v-model="item.settings.active" class="text-white w-full" label="Active" :disable="
              (item.t3Entry && item.t3Entry?.auto_manual === 0) ||
              item.t3Entry?.digital_analog === 1
            ">
            <q-tooltip v-if="item.t3Entry?.auto_manual === 0" anchor="center left" self="center end">
              Can't activate it because the linked entry is in auto mode
            </q-tooltip></q-checkbox>
          <q-checkbox dark filled v-model="item.settings.inAlarm" class="text-white w-full" label="In alarm"
      ...
      <div>
        ...
        <q-btn dark outline no-caps stretch :icon="item.t3Entry ? 'dataset_linked' : undefined" class="text-white w-full"
          :label="
            !item.t3Entry
              ? 'Link with an entry'
        ...
        <q-expansion-item v-if="item.t3Entry" class="mt-2 border border-solid border-gray-700" dark default-opened
          ...
          <div class="p-1">
            <q-select class="mb-1" filled dark v-model="item.t3Entry.auto_manual" :options="[
              { label: 'Auto', value: 0 },
              { label: 'Manual', value: 1 },
            ]" label="Auto/Manual" emit-value map-options
              @update:model-value="T3UpdateEntryField('auto_manual', item)" />
            <q-select class="mb-1" v-if="item.t3Entry.digital_analog === 0 && item.t3Entry.range"
            ...
            <!-- Program status -->
            <q-select class="mb-1" v-if="item.t3Entry.type === 'PROGRAM'" :disable="item.t3Entry?.auto_manual === 0"
            ...
            <!-- Analog range value -->
            <q-input class="mb-1" v-if="item.t3Entry.digital_analog === 1" :disable="item.t3Entry?.auto_manual === 0"
              filled dark type="number" v-model.number="item.t3Entry.value" label="Value"
