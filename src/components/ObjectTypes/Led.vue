<template>
  <div class="led-container">
    ...
    >
      ...
      <path
        d="m14.173 13.001v-5.9126c0-3.9132-3.168-7.0884-7.0855-7.0884-3.9125 0-7.0877 3.1694-7.0877 7.0884v13.649c1.4738 1.651 4.0968 2.7526 7.0877 2.7526 4.6195 0 8.3686-2.6179 8.3686-5.8594v-1.5235c-7.4e-4 -1.1426-0.47444-2.2039-1.283-3.1061z"
        :fill="(!blink && active) || (blink && blinkVal) ? onColor : offColor"
        opacity=".65"
      />
      <g fill="#ffffff">
        <path
      ...
      <g
        class="light"
        :style="`display: ${
          (!blink && active) || (blink && blinkVal) ? '' : 'none'
        }`"
      >
        <ellipse
          cx="8"
...
<script>
import { defineComponent, computed, watchEffect, ref, onUnmounted } from "vue";
...
export default defineComponent({
  ...
  setup(props) {
    const blinkVal = ref(false);
    const blinkIntervalRef = ref(null);
    function blinkValToggle() {
      if (blinkVal.value === false) {
        blinkVal.value = true;
      } else {
        blinkVal.value = false;
      }
    }
    watchEffect(() => {
      if (props.blink) {
      ...
      } else if (!props.blink && blinkIntervalRef.value) {
        clearInterval(blinkIntervalRef.value);
      }
    });
    onUnmounted(() => {
    ...
    const opacity = computed(() => {
      return props.brightness ? 0.3 + props.brightness * 0.7 : 0;
    });
    return { opacity, blinkVal };
  },
});
</script>
