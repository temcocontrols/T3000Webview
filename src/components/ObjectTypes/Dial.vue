<template>
  <div>
    <svg :style="$attrs.svgStyle" :class="' temco-linear-vertical temco-dial temco-dial-' + $attrs.size"
      :height="svgheight" :width="svgwidth" :view-box="'0 0 ' + svgwidth + ' ' + svgheight" :ref="$attrs.serial"
      :id="$attrs.serial">

      <g class="temco-scale" :ref="'scale-' + $attrs.serial" stroke="red" :style="scaleStyle"></g>

      <rect class="temco-outline " :id="'outline-' + $attrs.serial" :style="fillStyle" :x="offsetX"
        :height="svgheight - (offsetY * 2)" :width="barHeight" :y="offsetY">

      </rect>
      <rect class="temco-fill " :style="outlineStyle" :id="'fill-' + $attrs.serial" :x="offsetX" :height="pos"
        :width="barHeight" :y="offsetY">

      </rect>
      <rect :id="'needle-' + $attrs.serial" class="temco-needle" :style="animate('y')" height="1" :y="pos + offsetY"
        :x="offsetX" :width="barHeight" :fill="$attrs.needleColor" />

      <template v-for="(zone, i) in limitzones" :key="'zone-' + i">
        <rect class="temco-zones" :id="'zones-' + i + '-' + $attrs.serial" :fill="zone" :x="offsetX - 6"
          :height="zoneHeight" :width="5" :y="offsetY + (zoneHeight * i)" />
      </template>
...
<style scoped>
.temco-dial-linear-vertical-output {
  text-align: center;
  color: #2199de;
  font-weight: bold;
}
