/**
 * MobileRangeDrawer for Variables — bottom-sheet range selector.
 *
 ...
 *   • Variable Analog Units section: grouped by category
 */
...
export const MobileRangeDrawer: React.FC<MobileRangeDrawerProps> = ({
  ...
  // Variable analog sub-groups (matching PC layout: left col = 31–50, right col = 51–68)
  const analogGroup1 = VARIABLE_ANALOG_RANGES.filter(r => r.value >= 31 && r.value <= 50);
  const analogGroup2 = VARIABLE_ANALOG_RANGES.filter(r => r.value >= 51 && r.value <= 63);
  const analogCustom = VARIABLE_ANALOG_RANGES.filter(r => r.value >= 64 && r.value <= 68);
    ...
    >
      ...
      <DrawerBody className={styles.body}>
        ...
        <RadioGroup value={radioGroupValue} onChange={handleRadioChange} style={{ width: '100%' }}>
          ...
          <CollapsibleSection title="Variable Analog Units" defaultOpen={isAnalogCurrent}>
            {/* Temp / Power / Electrical / Time (31–50) */}
            <div className={styles.subSectionHeader}>
              <span className={styles.subSectionTitle}>Temp / Power / Electrical / Time</span>
            </div>
            <div className={styles.radioGrid}>
              {analogGroup1.map(r => (
                <div key={r.value} className={styles.radioItem}>
            ...
            {/* Measurement / Flow (51–63) */}
            <div className={styles.subSectionHeader}>
              <span className={styles.subSectionTitle}>Measurement / Flow</span>
            </div>
            <div className={styles.radioGrid}>
              {analogGroup2.map(r => (
                <div key={r.value} className={styles.radioItem}>
            ...
            {/* Custom (64–68) */}
            <div className={styles.subSectionHeader}>
              <span className={styles.subSectionTitle}>Custom</span>
            </div>
            <div className={styles.radioGrid}>
              {analogCustom.map(r => (
                <div key={r.value} className={styles.radioItem}>
