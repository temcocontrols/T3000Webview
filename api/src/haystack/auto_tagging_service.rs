...
pub async fn delete_rule(db: &impl ConnectionTrait, id: i64, force: bool) -> Result<(), String> {
    if force {
        ...
        if let Some(row) = rows.first() {
            ...
            // but only if no other rule (still in the DB) also assigns the same tag.
            if category == "haystack" {
                if let Some(ref tags) = haystack_tags {
                    for tag in tags.split(',') {
                        let tag = tag.trim();
                        if !tag.is_empty() {
                            // Count other rules that share this tag
                            let shared = db
                                .query_one(Statement::from_sql_and_values(
                                    sea_orm::DatabaseBackend::Sqlite,
                                    "SELECT COUNT(*) as cnt FROM HAYSTACK_AUTO_TAGGING_RULES WHERE id != ? AND haystack_tags LIKE ?",
                                    vec![id.into(), format!("%{}%", tag).into()],
                                ))
                                .await
                                .ok()
                                .flatten()
                                .and_then(|r| r.try_get::<i64>("", "cnt").ok());
                            if shared.unwrap_or(0) == 0 {
                                db.execute(Statement::from_sql_and_values(
                                    sea_orm::DatabaseBackend::Sqlite,
            ...
            // For brick / range rules: delete auto-assigned brick_class entries,
            // but only if no other rule (still in the DB) also assigns the same brick_class.
            if category == "brick" || category == "range" {
                if let Some(ref bc) = brick_class {
                    if !bc.is_empty() {
                        let shared = db
                            .query_one(Statement::from_sql_and_values(
                                sea_orm::DatabaseBackend::Sqlite,
                                "SELECT COUNT(*) as cnt FROM HAYSTACK_AUTO_TAGGING_RULES WHERE id != ? AND brick_class = ?",
                                vec![id.into(), bc.clone().into()],
                            ))
                            .await
                            .ok()
                            .flatten()
...
pub async fn get_rule_affected_points(
    ...
    // For brick / range rules: find points with auto-assigned brick_class matching this rule
    if category == "brick" || category == "range" {
        if let Some(ref bc) = brick_class {
            if !bc.is_empty() {
...
static BUILTIN_RULES: &[BuiltinRule] = &[
    ...
    // ── Extended rules from m20260718/m20260729 (YAML-sourced) ──
    BuiltinRule { rule_name: "brick:eaf", category: "brick", pattern: "(?i)(?<![A-Za-z])(eaf|exhaust[_ ]?air[_ ]?flow|ea[_ ]?flow)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Exhaust_Air_Flow_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:exhaust_damper", category: "brick", pattern: "(?i)(?<![A-Za-z])(exhaust[_ ]?damper|ed|relief[_ ]?damper)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Exhaust_Damper"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:econ_damper", category: "brick", pattern: "(?i)(?<![A-Za-z])(econ[_ ]?damper|economizer[_ ]?damper)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Economizer_Damper"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:filter_dp", category: "brick", pattern: "(?i)(?<![A-Za-z])(filter[_ ]?dp|filter[_ ]?pressure|filter[_ ]?diff)(?![A-Za-z])", units: Some("inWC,\"wc,inH2O,Pa,kPa"), object_types: None, haystack_tags: None, brick_class: Some("Filter_Differential_Pressure_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:filter_air_dp", category: "brick", pattern: "(?i)(?<![A-Za-z])(filter[_ ]?air[_ ]?dp|air[_ ]?filter[_ ]?dp)(?![A-Za-z])", units: Some("inWC,\"wc,inH2O,Pa,kPa"), object_types: None, haystack_tags: None, brick_class: Some("Filter_Differential_Pressure_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:filter_status", category: "brick", pattern: "(?i)(?<![A-Za-z])(filter[_ ]?(status|state|alarm))(?![A-Za-z])", units: None, object_types: Some("binary-input,binary-value"), haystack_tags: None, brick_class: Some("Filter_Status"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:enthalpy", category: "brick", pattern: "(?i)(?<![A-Za-z])(enthalpy)(?![A-Za-z])", units: Some("Btu/lb,kJ/kg"), object_types: None, haystack_tags: None, brick_class: Some("Enthalpy_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:dewpoint", category: "brick", pattern: "(?i)(?<![A-Za-z])(dew[_ ]?point|dewpoint|dp[_ ]?temp)(?![A-Za-z])", units: Some("degF,degC,°F,°C"), object_types: None, haystack_tags: None, brick_class: Some("Dewpoint_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:wetbulb", category: "brick", pattern: "(?i)(?<![A-Za-z])(wet[_ ]?bulb|wetbulb|wb[_ ]?temp)(?![A-Za-z])", units: Some("degF,degC,°F,°C"), object_types: None, haystack_tags: None, brick_class: Some("Wet_Bulb_Temperature_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:cooling_coil", category: "brick", pattern: "(?i)(?<![A-Za-z])(cooling[_ ]?coil|cc[_ ]?(temp|status)|clg[_ ]?coil)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Cooling_Coil"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:heating_coil", category: "brick", pattern: "(?i)(?<![A-Za-z])(heating[_ ]?coil|hc[_ ]?(temp|status)|htg[_ ]?coil)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Heating_Coil"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:heat_exchanger", category: "brick", pattern: "(?i)(?<![A-Za-z])(heat[_ ]?exchanger|hx|hex)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Heat_Exchanger"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:chw_valve", category: "brick", pattern: "(?i)(?<![A-Za-z])(chw[_ ]?valve|chilled[_ ]?water[_ ]?valve)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Chilled_Water_Valve"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:hw_valve", category: "brick", pattern: "(?i)(?<![A-Za-z])(hw[_ ]?valve|hot[_ ]?water[_ ]?valve)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Hot_Water_Valve"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:bypass_valve", category: "brick", pattern: "(?i)(?<![A-Za-z])(bypass[_ ]?valve|bp[_ ]?valve)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Bypass_Valve"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:bypass_damper", category: "brick", pattern: "(?i)(?<![A-Za-z])(bypass[_ ]?damper|bp[_ ]?damper)(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Bypass_Damper"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:chw_dp", category: "brick", pattern: "(?i)(?<![A-Za-z])(chw[_ ]?dp|chilled[_ ]?water[_ ]?diff|chw[_ ]?diff[_ ]?pressure)(?![A-Za-z])", units: Some("psi,kPa,inWC"), object_types: None, haystack_tags: None, brick_class: Some("Chilled_Water_Differential_Pressure_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:hw_dp", category: "brick", pattern: "(?i)(?<![A-Za-z])(hw[_ ]?dp|hot[_ ]?water[_ ]?diff|hw[_ ]?diff[_ ]?pressure)(?![A-Za-z])", units: Some("psi,kPa,inWC"), object_types: None, haystack_tags: None, brick_class: Some("Hot_Water_Differential_Pressure_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:pump_vfd", category: "brick", pattern: "(?i)(?<![A-Za-z])(pump[_ ]?(vfd|speed|hz))(?![A-Za-z])", units: None, object_types: None, haystack_tags: None, brick_class: Some("Pump_VFD"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:condenser_water_temp", category: "brick", pattern: "(?i)(?<![A-Za-z])(condenser[_ ]?water[_ ]?temp|cw[_ ]?temp|cond[_ ]?water[_ ]?temp)(?![A-Za-z])", units: Some("degF,degC,°F,°C"), object_types: None, haystack_tags: None, brick_class: Some("Condenser_Water_Temperature_Sensor"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "brick:zone_humidity_sp", category: "brick", pattern: "(?i)(?<![A-Za-z])(zone[_ ]?(humidity|rh)[_ ]?(setpoint|sp|stpt))(?![A-Za-z])", units: Some("%,percent"), object_types: None, haystack_tags: None, brick_class: Some("Zone_Air_Humidity_Setpoint"), haystack_kind: None, haystack_unit: None },
    BuiltinRule { rule_name: "hs:eaf", category: "haystack", pattern: "(?i)(?<![A-Za-z])(eaf|exhaust[_ ]?air[_ ]?flow|ea[_ ]?flow)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,exhaust,air,flow"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("cfm") },
    BuiltinRule { rule_name: "hs:exhaust_damper", category: "haystack", pattern: "(?i)(?<![A-Za-z])(exhaust[_ ]?damper|ed|relief[_ ]?damper)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,exhaust,damper,position"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:econ_damper", category: "haystack", pattern: "(?i)(?<![A-Za-z])(econ[_ ]?damper|economizer[_ ]?damper)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,economizer,damper,position"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:filter_dp", category: "haystack", pattern: "(?i)(?<![A-Za-z])(filter[_ ]?dp|filter[_ ]?pressure|filter[_ ]?diff)(?![A-Za-z])", units: Some("inWC"), object_types: None, haystack_tags: Some("point,sensor,filter,pressure,differential"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("inWC") },
    BuiltinRule { rule_name: "hs:filter_status", category: "haystack", pattern: "(?i)(?<![A-Za-z])(filter[_ ]?(status|state|alarm))(?![A-Za-z])", units: None, object_types: Some("binary-input,binary-value"), haystack_tags: Some("point,sensor,filter,status"), brick_class: None, haystack_kind: Some("Bool"), haystack_unit: None },
    BuiltinRule { rule_name: "hs:enthalpy", category: "haystack", pattern: "(?i)(?<![A-Za-z])(enthalpy)(?![A-Za-z])", units: Some("Btu/lb"), object_types: None, haystack_tags: Some("point,sensor,air,enthalpy"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("Btu/lb") },
    BuiltinRule { rule_name: "hs:dewpoint", category: "haystack", pattern: "(?i)(?<![A-Za-z])(dew[_ ]?point|dewpoint|dp[_ ]?temp)(?![A-Za-z])", units: Some("degF,degC,°F,°C"), object_types: None, haystack_tags: Some("point,sensor,air,dewpoint"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("°F") },
    BuiltinRule { rule_name: "hs:wetbulb", category: "haystack", pattern: "(?i)(?<![A-Za-z])(wet[_ ]?bulb|wetbulb|wb[_ ]?temp)(?![A-Za-z])", units: Some("degF,degC,°F,°C"), object_types: None, haystack_tags: Some("point,sensor,air,wetbulb"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("°F") },
    BuiltinRule { rule_name: "hs:cooling_coil", category: "haystack", pattern: "(?i)(?<![A-Za-z])(cooling[_ ]?coil|cc[_ ]?(temp|status)|clg[_ ]?coil)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,cooling,coil"), brick_class: None, haystack_kind: Some("Bool"), haystack_unit: None },
    BuiltinRule { rule_name: "hs:heating_coil", category: "haystack", pattern: "(?i)(?<![A-Za-z])(heating[_ ]?coil|hc[_ ]?(temp|status)|htg[_ ]?coil)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,heating,coil"), brick_class: None, haystack_kind: Some("Bool"), haystack_unit: None },
    BuiltinRule { rule_name: "hs:cooling_valve", category: "haystack", pattern: "(?i)(?<![A-Za-z])(cooling[_ ]?valve|cooling[_ ]?coil[_ ]?valve|ccv|cw[_ ]?valve[_ ]?pos)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,cooling,valve,position"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:heating_valve", category: "haystack", pattern: "(?i)(?<![A-Za-z])(heating[_ ]?valve|heating[_ ]?coil[_ ]?valve|hcv|hw[_ ]?valve[_ ]?pos)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,heating,valve,position"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:heat_exchanger", category: "haystack", pattern: "(?i)(?<![A-Za-z])(heat[_ ]?exchanger|hx|hex)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,heat,exchanger"), brick_class: None, haystack_kind: Some("Bool"), haystack_unit: None },
    BuiltinRule { rule_name: "hs:chw_valve", category: "haystack", pattern: "(?i)(?<![A-Za-z])(chw[_ ]?valve|chilled[_ ]?water[_ ]?valve)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,chilled,water,valve"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:hw_valve", category: "haystack", pattern: "(?i)(?<![A-Za-z])(hw[_ ]?valve|hot[_ ]?water[_ ]?valve)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,hot,water,valve"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:bypass_valve", category: "haystack", pattern: "(?i)(?<![A-Za-z])(bypass[_ ]?valve|bp[_ ]?valve)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,bypass,valve"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:bypass_damper", category: "haystack", pattern: "(?i)(?<![A-Za-z])(bypass[_ ]?damper|bp[_ ]?damper)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,bypass,damper,position"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
    BuiltinRule { rule_name: "hs:chw_dp", category: "haystack", pattern: "(?i)(?<![A-Za-z])(chw[_ ]?dp|chilled[_ ]?water[_ ]?diff|chw[_ ]?diff[_ ]?pressure)(?![A-Za-z])", units: Some("psi"), object_types: None, haystack_tags: Some("point,sensor,chilled,water,pressure,differential"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("psi") },
    BuiltinRule { rule_name: "hs:hw_dp", category: "haystack", pattern: "(?i)(?<![A-Za-z])(hw[_ ]?dp|hot[_ ]?water[_ ]?diff|hw[_ ]?diff[_ ]?pressure)(?![A-Za-z])", units: Some("psi"), object_types: None, haystack_tags: Some("point,sensor,hot,water,pressure,differential"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("psi") },
    BuiltinRule { rule_name: "hs:pump_vfd", category: "haystack", pattern: "(?i)(?<![A-Za-z])(pump[_ ]?(vfd|speed|hz))(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,pump,vfd,speed"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("Hz") },
    BuiltinRule { rule_name: "hs:compressor", category: "haystack", pattern: "(?i)(?<![A-Za-z])(compressor|comp[_ ]?(status|run|cmd|speed))(?![A-Za-z])", units: None, object_types: Some("binary-input,binary-output,binary-value,multi-state-input,multi-state-output"), haystack_tags: Some("point,sensor,compressor,run"), brick_class: None, haystack_kind: Some("Bool"), haystack_unit: None },
    BuiltinRule { rule_name: "hs:fan_vfd", category: "haystack", pattern: "(?i)(?<![A-Za-z])(fan[_ ]?(vfd|speed|hz)|vfd[_ ]?(speed|hz)|drive[_ ]?speed)(?![A-Za-z])", units: None, object_types: None, haystack_tags: Some("point,sensor,fan,vfd,speed"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("Hz") },
    BuiltinRule { rule_name: "hs:condenser_water_temp", category: "haystack", pattern: "(?i)(?<![A-Za-z])(condenser[_ ]?water[_ ]?temp|cw[_ ]?temp|cond[_ ]?water[_ ]?temp)(?![A-Za-z])", units: Some("degF,degC,°F,°C"), object_types: None, haystack_tags: Some("point,sensor,condenser,water,temp"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("°F") },
    BuiltinRule { rule_name: "hs:zone_humidity_sp", category: "haystack", pattern: "(?i)(?<![A-Za-z])(zone[_ ]?(humidity|rh)[_ ]?(setpoint|sp|stpt))(?![A-Za-z])", units: Some("%,percent"), object_types: None, haystack_tags: Some("point,sp,zone,air,humidity"), brick_class: None, haystack_kind: Some("Number"), haystack_unit: Some("%") },
];


static BUILTIN_RANGE_RULES: &[&str] = &[
    // ═══ INPUTS ═══
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-0','range',NULL,'point,sensor,binary',NULL,NULL,NULL,NULL,NULL,'INPUT',0,0,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-0','range',NULL,'point,sensor,analog',NULL,NULL,NULL,NULL,NULL,'INPUT',1,0,'migration',100)",
    // Analog ranges 1-34
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-1','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.C','Temperature','Number',NULL,'INPUT',1,1,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-2','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.F','Temperature','Number',NULL,'INPUT',1,2,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-3','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.C','Temperature','Number',NULL,'INPUT',1,3,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-4','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.F','Temperature','Number',NULL,'INPUT',1,4,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-5','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.C','Temperature','Number',NULL,'INPUT',1,5,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-6','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.F','Temperature','Number',NULL,'INPUT',1,6,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-7','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.C','Temperature','Number',NULL,'INPUT',1,7,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-8','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.F','Temperature','Number',NULL,'INPUT',1,8,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-9','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.C','Temperature','Number',NULL,'INPUT',1,9,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-10','range',NULL,'point,sensor,air,temp','Temperature_Sensor','Deg.F','Temperature','Number',NULL,'INPUT',1,10,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-11','range',NULL,'point,sensor,voltage',NULL,'Volts','Voltage','Number',NULL,'INPUT',1,11,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-12','range',NULL,'point,sensor,current','Current_Sensor','Amps','Current','Number',NULL,'INPUT',1,12,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-13','range',NULL,'point,sensor,current','Current_Sensor','ma','Current','Number',NULL,'INPUT',1,13,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-14','range',NULL,'point,sensor,pressure','Pressure_Sensor','psi','Pressure','Number',NULL,'INPUT',1,14,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-15','range',NULL,'point,sensor,pulse,counter',NULL,'counts','Pulse','Number',NULL,'INPUT',1,15,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-16','range',NULL,'point,sensor,percent','Percentage_Sensor','%','Percentage','Number',NULL,'INPUT',1,16,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-17','range',NULL,'point,sensor,percent','Percentage_Sensor','%','Percentage','Number',NULL,'INPUT',1,17,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-18','range',NULL,'point,sensor,percent','Percentage_Sensor','%','Percentage','Number',NULL,'INPUT',1,18,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-19','range',NULL,'point,sensor,voltage',NULL,'Volts','Voltage','Number',NULL,'INPUT',1,19,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-20','range',NULL,'point,sensor,custom',NULL,NULL,'Custom Tables',NULL,NULL,'INPUT',1,20,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-21','range',NULL,'point,sensor,custom',NULL,NULL,'Custom Tables',NULL,NULL,'INPUT',1,21,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-22','range',NULL,'point,sensor,custom',NULL,NULL,'Custom Tables',NULL,NULL,'INPUT',1,22,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-23','range',NULL,'point,sensor,custom',NULL,NULL,'Custom Tables',NULL,NULL,'INPUT',1,23,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-24','range',NULL,'point,sensor,custom',NULL,NULL,'Custom Tables',NULL,NULL,'INPUT',1,24,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-25','range',NULL,'point,sensor,pulse,counter',NULL,'counts','Pulse','Number',NULL,'INPUT',1,25,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-26','range',NULL,'point,sensor,frequency',NULL,'Hz','Frequency','Number',NULL,'INPUT',1,26,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-27','range',NULL,'point,sensor,air,humidity','Humidity_Sensor','%','Environmental','Number',NULL,'INPUT',1,27,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-28','range',NULL,'point,sensor,air,co2,concentration','CO2_Sensor','PPM','Environmental','Number',NULL,'INPUT',1,28,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-29','range',NULL,'point,sensor,speed',NULL,'RPM','Speed','Number',NULL,'INPUT',1,29,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-30','range',NULL,'point,sensor,air,tvoc,concentration',NULL,'PPB','Environmental','Number',NULL,'INPUT',1,30,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-31','range',NULL,'point,sensor,air,pm,concentration',NULL,'ug/m3','Environmental','Number',NULL,'INPUT',1,31,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-32','range',NULL,'point,sensor,air,particle,concentration',NULL,'#/cm3','Environmental','Number',NULL,'INPUT',1,32,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-33','range',NULL,'point,sensor,sound,level',NULL,'dB','Sound','Number',NULL,'INPUT',1,33,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-ana-34','range',NULL,'point,sensor,light,level','Luminance_Sensor','Lux','Light','Number',NULL,'INPUT',1,34,'migration',100)",
    // Digital input ranges
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-1','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,1,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-2','range',NULL,'point,sensor,contact,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,2,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-3','range',NULL,'point,sensor,run,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,3,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-4','range',NULL,'point,sensor,enable,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,4,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-5','range',NULL,'point,sensor,alarm,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,5,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-6','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,6,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-7','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,7,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-8','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,8,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-9','range',NULL,'point,sensor,mode,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,9,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-10','range',NULL,'point,sensor,occupancy,binary','Occupancy_Sensor',NULL,NULL,'Bool',NULL,'INPUT',0,10,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-11','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,11,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-12','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,12,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-13','range',NULL,'point,sensor,contact,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,13,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-14','range',NULL,'point,sensor,run,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,14,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-15','range',NULL,'point,sensor,enable,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,15,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-16','range',NULL,'point,sensor,alarm,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,16,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-17','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,17,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-18','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,18,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-19','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,19,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-20','range',NULL,'point,sensor,mode,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,20,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-21','range',NULL,'point,sensor,occupancy,binary','Occupancy_Sensor',NULL,NULL,'Bool',NULL,'INPUT',0,21,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:in-dig-22','range',NULL,'point,sensor,status,binary',NULL,NULL,NULL,'Bool',NULL,'INPUT',0,22,'migration',100)",
    // ═══ OUTPUTS ═══
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-0','range',NULL,'point,cmd,binary',NULL,NULL,NULL,NULL,NULL,'OUTPUT',0,0,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-0','range',NULL,'point,cmd,analog',NULL,NULL,NULL,NULL,NULL,'OUTPUT',1,0,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-31','range',NULL,'point,cmd,voltage',NULL,'Volts','Voltage','Number',NULL,'OUTPUT',1,31,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-32','range',NULL,'point,cmd,damper,position','Damper_Position_Actuator','%','Percentage','Number',NULL,'OUTPUT',1,32,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-33','range',NULL,'point,cmd,pressure','Pressure_Actuator','psi','Pressure','Number',NULL,'OUTPUT',1,33,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-34','range',NULL,'point,cmd,percent','Percentage_Command','%','Percentage','Number',NULL,'OUTPUT',1,34,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-35','range',NULL,'point,cmd,damper,position','Damper_Position_Actuator','%','Percentage','Number',NULL,'OUTPUT',1,35,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-36','range',NULL,'point,cmd,current','Current_Command','ma','Current','Number',NULL,'OUTPUT',1,36,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-37','range',NULL,'point,cmd,pwm',NULL,'%','PWM','Number',NULL,'OUTPUT',1,37,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-ana-38','range',NULL,'point,cmd,percent','Percentage_Command','%','Percentage','Number',NULL,'OUTPUT',1,38,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-1','range',NULL,'point,cmd,status,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,1,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-2','range',NULL,'point,cmd,contact,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,2,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-3','range',NULL,'point,cmd,run,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,3,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-4','range',NULL,'point,cmd,enable,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,4,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-5','range',NULL,'point,cmd,alarm,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,5,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-9','range',NULL,'point,cmd,mode,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,9,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-10','range',NULL,'point,cmd,occupancy,binary','Occupancy_Command',NULL,NULL,'Bool',NULL,'OUTPUT',0,10,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-12','range',NULL,'point,cmd,status,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,12,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-13','range',NULL,'point,cmd,contact,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,13,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-14','range',NULL,'point,cmd,run,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,14,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-15','range',NULL,'point,cmd,enable,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,15,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-20','range',NULL,'point,cmd,mode,binary',NULL,NULL,NULL,'Bool',NULL,'OUTPUT',0,20,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:out-dig-21','range',NULL,'point,cmd,occupancy,binary','Occupancy_Command',NULL,NULL,'Bool',NULL,'OUTPUT',0,21,'migration',100)",
    // ═══ VARIABLES ═══
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-dig-0','range',NULL,'point,sp,binary',NULL,NULL,NULL,NULL,NULL,'VARIABLE',0,0,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-0','range',NULL,'point,sp,analog',NULL,NULL,NULL,NULL,NULL,'VARIABLE',1,0,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-31','range',NULL,'point,sp,temp',NULL,'Deg.C','Temperature','Number',NULL,'VARIABLE',1,31,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-32','range',NULL,'point,sp,temp',NULL,'Deg.F','Temperature','Number',NULL,'VARIABLE',1,32,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-33','range',NULL,'point,sp,velocity',NULL,'Feet per Min','Velocity','Number',NULL,'VARIABLE',1,33,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-34','range',NULL,'point,sp,pressure',NULL,'Pascals','Pressure','Number',NULL,'VARIABLE',1,34,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-35','range',NULL,'point,sp,pressure',NULL,'KPascals','Pressure','Number',NULL,'VARIABLE',1,35,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-36','range',NULL,'point,sp,pressure',NULL,'psi','Pressure','Number',NULL,'VARIABLE',1,36,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-37','range',NULL,'point,sp,pressure',NULL,'inches WC','Pressure','Number',NULL,'VARIABLE',1,37,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-38','range',NULL,'point,sp,power',NULL,'Watts','Power','Number',NULL,'VARIABLE',1,38,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-39','range',NULL,'point,sp,power',NULL,'KWatts','Power','Number',NULL,'VARIABLE',1,39,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-40','range',NULL,'point,sp,energy',NULL,'KWH','Energy','Number',NULL,'VARIABLE',1,40,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-41','range',NULL,'point,sp,voltage',NULL,'Volts','Electrical','Number',NULL,'VARIABLE',1,41,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-42','range',NULL,'point,sp,voltage',NULL,'KV','Electrical','Number',NULL,'VARIABLE',1,42,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-43','range',NULL,'point,sp,current',NULL,'Amps','Electrical','Number',NULL,'VARIABLE',1,43,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-44','range',NULL,'point,sp,current',NULL,'ma','Electrical','Number',NULL,'VARIABLE',1,44,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-45','range',NULL,'point,sp,air,flow',NULL,'CFM','Flow','Number',NULL,'VARIABLE',1,45,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-46','range',NULL,'point,sp,time',NULL,'Seconds','Time','Number',NULL,'VARIABLE',1,46,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-47','range',NULL,'point,sp,time',NULL,'Minutes','Time','Number',NULL,'VARIABLE',1,47,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-48','range',NULL,'point,sp,time',NULL,'Hours','Time','Number',NULL,'VARIABLE',1,48,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-49','range',NULL,'point,sp,time',NULL,'Days','Time','Number',NULL,'VARIABLE',1,49,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-50','range',NULL,'point,sp,time',NULL,'Time','Time','Number',NULL,'VARIABLE',1,50,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-51','range',NULL,'point,sp,resistance',NULL,'Ohms','Electrical','Number',NULL,'VARIABLE',1,51,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-52','range',NULL,'point,sp,percent',NULL,'%','Percentage','Number',NULL,'VARIABLE',1,52,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-53','range',NULL,'point,sp,humidity',NULL,'%RH','Humidity','Number',NULL,'VARIABLE',1,53,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-54','range',NULL,'point,sp,speed',NULL,'p/min','Speed','Number',NULL,'VARIABLE',1,54,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-55','range',NULL,'point,sp,counter',NULL,'Counts','Count','Number',NULL,'VARIABLE',1,55,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-56','range',NULL,'point,sp,damper,position',NULL,'%Open','Percentage','Number',NULL,'VARIABLE',1,56,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-57','range',NULL,'point,sp,mass',NULL,'Kg','Mass','Number',NULL,'VARIABLE',1,57,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-58','range',NULL,'point,sp,flow',NULL,'L/Hour','Flow','Number',NULL,'VARIABLE',1,58,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-59','range',NULL,'point,sp,flow',NULL,'GPH','Flow','Number',NULL,'VARIABLE',1,59,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-60','range',NULL,'point,sp,volume',NULL,'GAL','Volume','Number',NULL,'VARIABLE',1,60,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-61','range',NULL,'point,sp,volume',NULL,'CF','Volume','Number',NULL,'VARIABLE',1,61,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-62','range',NULL,'point,sp,energy',NULL,'BTU','Energy','Number',NULL,'VARIABLE',1,62,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-63','range',NULL,'point,sp,flow',NULL,'CMH','Flow','Number',NULL,'VARIABLE',1,63,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-64','range',NULL,'point,sp,custom',NULL,NULL,'Custom',NULL,NULL,'VARIABLE',1,64,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-65','range',NULL,'point,sp,custom',NULL,NULL,'Custom',NULL,NULL,'VARIABLE',1,65,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-66','range',NULL,'point,sp,custom',NULL,NULL,'Custom',NULL,NULL,'VARIABLE',1,66,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-67','range',NULL,'point,sp,custom',NULL,NULL,'Custom',NULL,NULL,'VARIABLE',1,67,'migration',100)",
    "INSERT OR IGNORE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,haystack_tags,brick_class,units,object_types,haystack_kind,haystack_unit,point_type,digital_analog,range_value,source,priority) VALUES ('range:var-ana-68','range',NULL,'point,sp,custom',NULL,NULL,'Custom',NULL,NULL,'VARIABLE',1,68,'migration',100)",
];
        ...
        if let Ok(yaml) = resp.text().await {
            if let Ok(parsed) = serde_yaml::from_str::<Vec<YamlRule>>(&yaml) {
                for r in parsed {
                    ...
                    let sql = format!(
                        "INSERT OR REPLACE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,units,object_types,brick_class,priority) VALUES ('{}','brick','{}',{},{},{},{})",
        ...
        if let Ok(yaml) = resp.text().await {
            if let Ok(parsed) = serde_yaml::from_str::<Vec<YamlRule>>(&yaml) {
                for r in parsed {
                    ...
                    let sql = format!(
                        "INSERT OR REPLACE INTO HAYSTACK_AUTO_TAGGING_RULES (rule_name,category,pattern,units,object_types,haystack_tags,haystack_kind,haystack_unit,priority) VALUES ('{}','haystack','{}',{},{},{},{},{},{})",
                        name.replace('\'', "''"), r.pattern.replace('\'', "''"),
