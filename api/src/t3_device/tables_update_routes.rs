...
pub async fn update_table_full(
    ...
    Json(payload): Json<UpdateTableFullRequest>,
) -> Result<Json<Value>, (StatusCode, String)> {
    let index = index_str.parse::<i32>().unwrap_or(0);
    info!("UPDATE_WEBVIEW_LIST: Updating full table record - Serial: {}, Index: {}", serial, index);
    ...
    if payload.table_name.is_some() {
        updated_fields.push("tableName");
    }
    ...
    let input_json = json!({
        ...
        "entryIndex": index,
        "table_name": payload.table_name.unwrap_or_default(),
    });

    // Call FFI function
    match call_update_ffi(WebViewMessageType::UPDATE_WEBVIEW_LIST as i32, input_json).await {
        Ok(_response) => {
            info!("✅ Full table record updated successfully");
            Ok(Json(json!({
                "success": true,
                "message": "Table point updated successfully",
                "data": {
                    "serialNumber": serial,
                    "tableIndex": index,
        ...
        Err(e) => {
            ...
            if e.contains("not implemented") || e.contains("empty response") {
                return Err((
                    ...
                    "UPDATE_WEBVIEW_LIST (Action 16) for tables is not yet implemented in C++. Please implement BacnetWebView_HandleWebViewMsg case 16 in T3000.exe".to_string(),
                ));
            }
