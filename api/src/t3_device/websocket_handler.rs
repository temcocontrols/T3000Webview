...
use crate::t3_device::logging_data_service;
...
async fn create_logging_data_receiver() -> broadcast::Receiver<String> {
    ...
    if let Some(_service) = logging_data_service::get_logging_service() {
...
async fn handle_websocket_command(command: &str, _json: &serde_json::Value) -> Result<(), ()> {
    match command {
        "request_sync" => {
            ...
            if let Some(service) = logging_data_service::get_logging_service() {
                tokio::spawn(async move {
